...
let statusFilter = 'all'; // Track filter state: 'all', 'active', 'offline'

// OpenSubtitles API credentials (replace with your own from opensubtitles.com)
const OPENSUBTITLES_API_KEY = 'YOUR_API_KEY_HERE'; // Replace with your key
const OPENSUBTITLES_USERNAME = 'YOUR_USERNAME'; // Replace with your username
const OPENSUBTITLES_PASSWORD = 'YOUR_PASSWORD'; // Replace with your password
...
async function fetchSubtitlesOnline(title) {
    try {
        ...
        const searchResponse = await fetch(`https://api.opensubtitles.com/api/v1/subtitles?query=${encodeURIComponent(title)}&languages=en`, {
            method: 'GET',
            headers: {
                'Api-Key': OPENSUBTITLES_API_KEY,
        ...
        if (searchData.data && searchData.data.length > 0) {
            const subtitleUrl = searchData.data[0].attributes.files[0].file_url;
            showSubtitlePrompt(subtitleUrl, title);
        } else {
            showNotification('No subtitles found online.');
        }
    } catch (error) {
        console.error('Error fetching subtitles:', error);
        showNotification('Failed to fetch subtitles online.');
    }
}

function showSubtitlePrompt(subtitleUrl, title) {
    notification.innerHTML = `Subtitles found for "${title}". Apply? 
        <button onclick="applySubtitle('${subtitleUrl}')">Yes</button>
        <button class="no" onclick="hideNotification()">No</button>`;
    notification.classList.add('with-buttons');
    notification.style.display = 'block';
}

async function applySubtitle(subtitleUrl) {
    try {
        ...
        hideNotification();
        showNotification('Subtitles applied!');
    } catch (error) {
        console.error('Error applying subtitle:', error);
        showNotification('Failed to apply subtitles.');
    }
}
...
function filterByStatus(status) {
    statusFilter = status;
    updatePlaylistStatus(); // Refresh display with new filter
}
